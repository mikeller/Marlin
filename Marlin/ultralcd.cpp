...
uint8_t lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW; // Set when the LCD needs to draw, decrements after every draw. Set to 2 in LCD routines so the LCD gets at least 1 full redraw (first redraw is partial)
  ...
  #define START_MENU() \
    ...
    bool wasClicked = LCD_CLICKED || REPRAPWORLD_KEYPAD_MOVE_X_RIGHT; \
      ...
      slow_buttons = lcd_implementation_read_slow_buttons(); // buttons which take too long to read in interrupt context
		...
		if (buttons_reprapworld_keypad != 0)
		{
			...
			if ((currentScreen == lcd_main_menu) || (currentScreen == lcd_tune_menu) || (currentScreen == lcd_prepare_menu) || (currentScreen == lcd_control_menu)
				...
				|| (currentScreen == lcd_control_temperature_menu) || (currentScreen == lcd_control_temperature_preheat_pla_settings_menu) || (currentScreen == lcd_control_temperature_preheat_abs_settings_menu)\
